...
const createProduct = async (db, productData) => {
  try {
    const { nome, descricao, preco, quantidade = 1 } = productData; // Agora inclui quantidade
    ...
    if (!existingProductQuery.empty) {
      ...
      const newQuantity = (existingProduct.data().quantidade || 0) + quantidade; // Incrementa a quantidade
      await db.collection('products').doc(existingProduct.id).update({ quantidade: newQuantity });
      return { id: existingProduct.id, nome, descricao, preco, quantidade: newQuantity }; // Retorna os dados atualizados
    }
    ...
    const productRef = await db.collection('products').add({
      ...
      descricao,
      preco,
      quantidade // Inicializa a quantidade conforme fornecida
    });

    return { id: productRef.id, nome, descricao, preco, quantidade }; // Retorna os dados do novo produto
  } catch (error) {
...
const updateProduct = async (db, productId, productData) => {
  try {
    const { nome, descricao, preco, quantidade } = productData; // Agora inclui quantidade

    const updatedData = {};
    if (nome) updatedData.nome = nome;
    if (descricao) updatedData.descricao = descricao;
    if (typeof preco === 'number') updatedData.preco = preco;
    if (typeof quantidade === 'number') updatedData.quantidade = quantidade; // Atualiza a quantidade
